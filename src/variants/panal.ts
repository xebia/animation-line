...
export const panal: Variant = {
  ...
  generate({ t, W, H, lineCount }: VariantEnv): Polyline[] {
    const cell = Math.max(W, H) / (lineCount ?? 6);
    const out: Polyline[] = [];

    for (const tile of hexTiles(W, H, cell)) {
      const { cx, cy } = tile;
      const b = breathe(cx, cy, t, W, H, { scaleAmp: 0.06, rotAmp: 0.16, freq: 0.9 });
      ...
      for (let k = 0; k < RINGS; k++) {
        ...
        const pulse = 1 + 0.06 * Math.sin(k * 0.9 - t * 0.0016 + (tile.i + tile.j) * 0.4);
        ...
        out.push({ pts, s: b.s, a: 0.85, w: 1.0 });
      }
